...
function uefc_options() {
    ...
    ?>
<div class="wrap">
<h1><?php echo esc_html__( 'Upcoming Events for Calendly Settings' ); ?></h1>
    <?php
    ...
    if( isset($_POST[ $hidden_field_name ]) && $_POST[ $hidden_field_name ] == 'uefc_settings') {
        ...
        if ($uefc_apikey) {
            ...
            if (property_exists($data, 'message')) {
                ?><div class="notice notice-error settings-error"><p><strong><?php esc_html_e( 'The Access Token you supplied is not valid. It was not saved.' ); ?></strong></p></div><?php
            } else {
                update_option('uefc_apikey', $uefc_apikey);
                ?><div class="updated"><p><strong><?php esc_html_e( 'Access Token successfully updated.' ); ?></strong></p></div><?php
            }
        }
    ...
    if (get_option("uefc_apikey")) {
        ...
        if (property_exists($data, 'message')) {
            ?><div class="notice notice-error settings-error"><p><strong><?php esc_html_e( 'Your current Access Token is not valid.' ); ?></strong></p></div><?php
        } else {
            ?><div class="updated"><p><?php esc_html_e( 'Your current Access Token is valid. You are logged in as:' ); ?><br>
...
<tbody>
<tr>
  <th scope="row"><label for="uefc_apikey"><?php esc_html_e( 'Calendly Access Token' ); ?></label></th>
  <td><?php
    if (get_option("uefc_apikey")) {
       ?><?php esc_html_e( 'For security reasons your existing Access Token is not shown here. To change it, paste a new one below.' ); ?><br><?php
    }
  ?><textarea id="uefc_apikey" name="uefc_apikey" class="regular-text code" rows="3"></textarea>
  <p class="description"><?php esc_html_e( 'Generate an Access Token on' ); ?> <a href="https://calendly.com/integrations/api_webhooks" target="_blank" class="external"><?php esc_html_e( 'this page' ); ?></a>, <?php esc_html_e( 'then enter it here.' ); ?></p>
  </td>
</tr>
</tbody>
</table>
<p class="submit"><input id="submit" class="button button-primary" type="submit" value="<?php esc_attr_e( 'Save Changes' ); ?>" name="submit"></p>
</form>
<p><?php esc_html_e( 'To place a list of your upcoming events that have already been scheduled into a post or page, use the shortcode' ); ?> <code>[upcoming-for-calendly]</code>. <?php esc_html_e( 'To restrict it to a specific event type, pass the title of the event (must be an exact match) like so:' ); ?> <code>[upcoming-for-calendly event="Event Name"]</code>.</p>
<?php
    echo "</div>";
}
